...
//Some more globals that I have defined 
int fb_fd;
const char *fb_path = NULL;
char *fb_mem = NULL;

//Init framebuffer
int init_framebuffer(const char **fb_path) {
    int fb_fd;
    
    // Try different frame buffer device files, starting with /dev/fb1 (common for Sense HAT)
    const char *possible_fbs[] = {"/dev/fb1", "/dev/fb0", "/dev/fb2"};
    for (int i = 0; i < 3; i++) {
        fb_fd = open(possible_fbs[i], O_RDWR);
        if (fb_fd != -1) {
            *fb_path = possible_fbs[i];  // Store the path to the found frame buffer
            printf("Found frame buffer at %s\n", possible_fbs[i]);
            return fb_fd;  // Return the file descriptor of the frame buffer
        }
    }
...
int init_joystick(const char **js_path) //TODO:ADD THIS TO RASPBERRY
{
    int js_fd;
    const char *possible_js("/dev/input/event0", "/dev/input/event1", "/dev/input/event2", "/dev/input/event3");
    //Check all possible joystick paths
    for(int i = 0; i < 4; i++){
        js_fd = open(possible_js[i], O_RDONLY);
        if(js_fd == -1){
            continue; //If the file can not be opened, try the next one
        }
        char joystick_name[256] = "";
        if(ioctl(js_fd, EVIOCGNAME(sizeof(joystick_name)), joystick_name) < 0){
            close(js_fd);
            continue; //If we fail to get the device name, try the next one
        }
        if(strcmp(joystick_name, "Raspberry Pi Sense HAT Joystick") == 0){
            *js_path = possible_js[i];
            printf("Found joystick at %s\n", possible_js[i]);
            return js_fd;
        }
        close(js_fd); //We are not interested in this joystick
    }
    return -1;
}
...
{
   // const char *fb_path; //Maybe move this to global
   fb_fd = init_framebuffer(&fb_path);
   //Check error
   if(fb_fd == -1){
   return false;
   }
   ...
   {
    for(int y = 0; y < 8; y++){
	((unsigned short *)fb_mem)[x + y * 8] = 0x0000;
       }
    }
   return true;
}
...
void freeSenseHat()
{
   long screensize = 8*8; //TODO:FIX THIS
...
int readSenseHatJoystick(struct input_event ev, int js_fd)
{
    //Read the joystick
    //TODO:MAKE THIS 
    return 0;
...
//moved this donw here since it calles on tileOppcupied()
void renderSenseHatMatrix(bool const playfieldChanged)
{
     // (void)playfieldChanged;//Think I can sefely can remove this
    //Need to creata a sort of nested foor loop that iterates thru the 1d array like it was 2d THIS DOES NOT WORK TODO
    for(unsigned int y = 0; y < game.grid.y; y++){ //TODO CHANGE THIS TO BE NUMBER OF PIXELS    
    	for(unsigned int x = 0; x < game.grid.x; x++){
	coord const checkTile = {x, y};
	if (tileOccupied(checkTile)) ((unsigned short *)fb_mem)[y*8 + x] = 0xF800;
	else ((unsigned short *)fb_mem)[y*8 + x] = 0x0000;
	}
    }
 }
...
int main(int argc, char **argv)
{
    ...
    //Code to init the joystick on the sense hat //TODO:ADD THIS TO RASPBERRY
    const char *js_path;
    int js_fd = init_joystick(&js_path);
    if(js_fd == -1){
        printf("Error: Could not find joystick\n");
        return false;
    }
    //Input event struct
    ...
    while (true)
    {
        ...
        int key = readSenseHatJoystick(&ev, js_fd);
        if (!key)
        {
